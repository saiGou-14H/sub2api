...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	codexTicketSettingsRepository := repository.NewCodexTicketSettingsRepository(db)
	codexTicketRuntimeStore := repository.NewCodexTicketCache(redisClient)
	codexTicketRuntime := service.NewCodexTicketRuntime(codexTicketSettingsRepository, proxyRepository, accountRepository, codexTicketRuntimeStore)
	codexTicketHandler := handler.ProvideCodexTicketHandler(codexTicketSettingsRepository, proxyRepository, codexTicketRuntime, openAIGatewayService)
	...
	webCodexRunner, err := server.ProvideWebCodexRunner(configConfig, db, userRepository)
	if err != nil {
		return nil, err
	}
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, optionalJWTAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, auditLogMiddleware, stepUpAuthMiddleware, apiKeyService, subscriptionService, opsService, settingService, compositeRouteResolver, redisClient, webCodexRunner)
	httpServer := server.ProvideHTTPServer(configConfig, engine, webCodexRunner)
