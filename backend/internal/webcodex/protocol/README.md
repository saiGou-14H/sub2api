...
Outer `RunnerRequest` positional arrays and general non-Job serde alternate forms remain prior gaps. Only the two raw Job carriers widen to object-or-array; other opaque slots retain their existing shape checks. Carrier parsing preserves raw data; call the typed Job read for structural validation. The existing sixteen DTO discriminator strings (plus the separate canonical operations table) and six-operation synchronous consumer remain bounded pure codecs, with capability bits unchanged. `DecodeJobRequest` additionally accepts positional `process` structs through its own field decoder; generic `ReadRequest` and synchronous process parsing keep their prior object-only behavior.
