...
// runner_operation.rs:218-228,298,943-969,1372-1413. Outer codec only.
package protocol
...
func TestApplyTextEditsOpaqueRoundtrip(t *testing.T) {
	// Changes, selectors, matching and write policy remain opaque executor data.
	for _, content := range []*string{nil, ptr(""), ptr(`null`), ptr(`not JSON`), ptr(`{}`), ptr(`{"changes":[]}`), ptr(`{"changes":[{"path":"../outside","edits":[{"old_text":"目录/😀","new_text":"changed","occurrence":18446744073709551615,"line_scope":{"start_line":1,"end_line":2}}]}]}`), ptr(`{"changes":[],"changes":[{}]}`), ptr(`[[]]`), ptr("a\x00b")} {
		r := request(t, "file_apply_text_edits")
		r.Path, r.Cwd, r.Content, r.MaxBytes = ptr("目录/😀"), ptr("/项目"), content, ptr(^uint64(0))
		...
		want := FilePayload{Cwd: ptr("/项目"), Path: "目录/😀", Content: content, MaxBytes: ptr(^uint64(0))}
		if !ok || file.WireKind() != r.Kind || !reflect.DeepEqual(file.Payload, want) || inv.Metadata != (InvocationMetadata{r.RequestID, r.ClientID, r.RequestedBy, r.CreatedAt}) {
			t.Fatalf("canonical edits changed: %+v", inv)
		}
		...
		*decoded.Path, *decoded.Cwd, *decoded.MaxBytes = "changed", "changed", 0
		...
		if !reflect.DeepEqual(file.Payload, want) {
			t.Fatal("DTO mutation retargeted canonical edits")
		}
	}
...
func TestApplyTextEditsCompatibility(t *testing.T) {
	base := `{"request_id":"r","client_id":"c","kind":"file_apply_text_edits","command":"","timeout_secs":0,"requested_by":"alice","created_at":0,"path":"."}`
	for _, extra := range []string{``, `,"content":null,"cwd":null,"expected_sha256":null,"expected_prefix":null,"start_line":null,"end_line":null,"create_dirs":false`, `,"changes":[{}],"occurrence":1,"line_scope":{},"future":true`, `,"cwd":"","content":"{bad options"`} {
	...
	for name, change := range map[string]func(*RunnerRequest){
		...
		t.Run(name, func(t *testing.T) {
			...
			if _, err := DecodeRequest(wire); err == nil || errors.Is(err, ErrUnsupported) || errors.Is(err, ErrUnknownKind) {
				t.Fatalf("public expected invalid: %v", err)
			}
		})
	}
	for _, extra := range []string{`,"content":{}`, `,"content":null,"content":null`, `,"content":"\ud800"`, `,"max_bytes":18446744073709551616`, `,"max_bytes":-1`, `,"max_bytes":1.5`, `,"max_bytes":1e1`, `,"start_line":18446744073709551616`, `,"end_line":-1`} {
		if _, err := ReadRequest([]byte(base[:len(base)-1] + extra + `}`)); err == nil {
			t.Fatalf("invalid outer wire accepted: %s", extra)
		}
	}
	for _, field := range []string{"timeout_secs", "created_at"} {
		for _, value := range []string{"null", "1.5", "1e1", "18446744073709551616", "-9223372036854775809"} {
			wire := strings.Replace(base, `"`+field+`":0`, `"`+field+`":`+value, 1)
			if _, err := ReadRequest([]byte(wire)); err == nil {
				t.Fatalf("invalid numeric wire accepted: %s=%s", field, value)
			}
		}
	}
	unknown := strings.Replace(base, "file_apply_text_edits", "file_future_edits", 1)
