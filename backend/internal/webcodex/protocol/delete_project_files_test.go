...
// runner_operation.rs:283-292,943-969,1372-1413. Outer codec only.
package protocol
...
func TestDeleteProjectFilesOpaqueRoundtrip(t *testing.T) {
	// Inner paths, count, duplicate keys and deletion policy belong to execution.
	for _, content := range []*string{nil, ptr(""), ptr(`null`), ptr(`not JSON`), ptr(`{}`), ptr(`{"paths":[]}`), ptr(`{"paths":["../outside","目录/😀"],"future":true}`), ptr(`{"paths":[],"paths":["a"]}`), ptr(`["a"]`), ptr("a\x00b")} {
		r := request(t, "file_delete_project_files")
		...
		if !ok || file.WireKind() != r.Kind || !reflect.DeepEqual(file.Payload, want) || inv.Metadata != (InvocationMetadata{r.RequestID, r.ClientID, r.RequestedBy, r.CreatedAt}) {
			t.Fatalf("canonical delete changed: %+v", inv)
		}
		...
		*decoded.Path, *decoded.Cwd, *decoded.MaxBytes = "changed", "changed", 0
		...
		if !reflect.DeepEqual(file.Payload, want) {
			t.Fatal("DTO mutation retargeted canonical delete")
		}
	}
...
func TestDeleteProjectFilesCompatibility(t *testing.T) {
	base := `{"request_id":"r","client_id":"c","kind":"file_delete_project_files","command":"","timeout_secs":0,"requested_by":"alice","created_at":0,"path":"."}`
	for _, extra := range []string{``, `,"content":null,"cwd":null,"expected_sha256":null,"expected_prefix":null,"start_line":null,"end_line":null,"create_dirs":false`, `,"paths":["a"],"future":true`, `,"cwd":"","content":"{bad options"`} {
