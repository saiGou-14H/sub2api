...
No real PostgreSQL DDL, foreign-key enforcement or concurrent database transaction behavior is claimed by these tests. Test rows and token strings are fabricated. The migration has not been applied to a database. Public issuance/revocation/import endpoints, UI, Wire provider registration, default-off router mounting, other credential families, persistent registry/results and full G2 dispatch remain later features. Applying the normal migration creates an empty credential table but enables no route and seeds no credential.
