...
// API keys use the standard endpoint; Codex OAuth reuses its authenticated cache.
// Web accounts use the browser entitlement catalog. Group policy is applied after
// discovery, while account mappings apply only to non-Web catalogs.
func (s *OpenAIGatewayService) FetchOpenAIModelsList(ctx context.Context, account *Account) (*OpenAIModelsResponse, error) {
	...
	if account.IsOpenAIWebTransport() {
		// Web entitlements come from the browser catalog, independently of the
		// credential lifecycle type and the Codex/API-key model cache.
		token, _, err := s.GetAccessToken(ctx, credentialAccount)
		if err != nil {
			return nil, fmt.Errorf("get Web model list access token: %w", err)
		}
		snapshot, err := s.newOpenAIWebTransport().DiscoverOpenAIWebModelCatalog(ctx, account, token)
		if err != nil {
			return nil, err
		}
