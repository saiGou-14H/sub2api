...
func TestOpenAIOAuth429_NonmatchingModelTempRuleKeepsAccountRuntimeBlock(t *testing.T) {
	...
	require.False(t, svc.isOpenAIAccountRuntimeBlocked(account))
	require.True(t, svc.shouldRetryOpenAIOAuth429OnSameAccount(account, http.StatusTooManyRequests, false))
	// The unmatched model rule must not block the requested model. The fork
	// still records the independent Codex transport cooldown for a generic 429.
	require.Len(t, repo.modelRateLimitCalls, 1)
	require.Equal(t, openAICodexTransportRateLimitKey, repo.modelRateLimitCalls[0].scope)
	require.Equal(t, "codex_no_reset_time", repo.modelRateLimitCalls[0].reason)
}
