...
func TestPinnedOpenAIModelsListWebUsesBrowserCatalogAndIgnoresMappings(t *testing.T) {
	for _, accountType := range []string{AccountTypeOAuth, AccountTypeSetupToken} {
		for _, staleMapping := range []bool{false, true} {
			name := accountType + "/unmapped"
			if staleMapping {
				name = accountType + "/stale_mapping"
			}
			t.Run(name, func(t *testing.T) {
				...
				upstream := &codexModelsHTTPUpstreamStub{do: func(req *http.Request, proxy string, accountID int64, _ int) (*http.Response, error) {
					calls.Add(1)
					assert.Equal(t, OpenAIWebModelsRoute, req.URL.Path)
					assert.Equal(t, OpenAIWebModelsPath, req.URL.RequestURI())
					assert.False(t, req.URL.Query().Has("client_version"))
					assert.Equal(t, "Bearer web-model-token", req.Header.Get("Authorization"))
					assert.EqualValues(t, 7, accountID)
					assert.Equal(t, "socks5h://127.0.0.1:7897", proxy)
					return ordinaryModelsUpstreamResponse(`{"models":[{"slug":"gpt-5.6-sol-wm","is_work_mode_model":true}]}`), nil
				}}
				proxyID := int64(3)
				account := Account{ID: 7, Platform: PlatformOpenAI, Type: accountType, Status: StatusActive, Schedulable: true,
					Credentials: map[string]any{"access_token": "web-model-token"},
					Extra:       map[string]any{OpenAIWebTransportExtraKey: OpenAITransportWeb},
					ProxyID:     &proxyID, Proxy: &Proxy{Protocol: "socks5h", Host: "127.0.0.1", Port: 7897}}
				if staleMapping {
					account.Credentials["model_mapping"] = map[string]any{"old-alias": "gpt-5.6-sol-wm"}
				}
				s := &OpenAIGatewayService{httpUpstream: upstream,
					accountRepo: splitCodexModelsAccountRepo{all: map[int64][]Account{10: {account}}}}
				s.openAIWebTransportFactory = func() *OpenAIWebTransport {
					return NewOpenAIWebTransportFromUpstream(upstream, OpenAIWebTransportOptions{SkipBootstrap: true})
				}
				...
				response, selected, err := s.FetchPinnedOpenAIModelsList(context.Background(), group, 3, "")
				require.NoError(t, err)
				require.EqualValues(t, 7, selected.ID)
				require.JSONEq(t, `{"object":"list","data":[{"id":"auto","object":"model","created":0,"owned_by":"openai"},{"id":"gpt-5.6-sol-wm","object":"model","created":0,"owned_by":"openai"}]}`, string(response.Body))
				require.NotEmpty(t, response.ETag)

				// Group policy still filters the original Web names, and client
				// ETag handling still works after mapping is bypassed.
				group.ModelAllowlist = GroupModelAllowlist{Enabled: true, Models: []string{"gpt-5.6-sol-wm"}}
				filtered, _, err := s.FetchPinnedOpenAIModelsList(context.Background(), group, 3, "")
				require.NoError(t, err)
				require.JSONEq(t, `{"object":"list","data":[{"id":"gpt-5.6-sol-wm","object":"model","created":0,"owned_by":"openai"}]}`, string(filtered.Body))
				again, _, err := s.FetchPinnedOpenAIModelsList(context.Background(), group, 3, filtered.ETag)
				require.NoError(t, err)
