...
func TestGetAvailableModelsUnmappedWebAccountsKeepWebCatalog(t *testing.T) {
	for _, accountType := range []string{AccountTypeOAuth, AccountTypeSetupToken} {
		t.Run(accountType, func(t *testing.T) {
			...
			account.SetOpenAIWebModelCatalogSnapshot(OpenAIWebModelCatalogSnapshot{Models: []string{"auto", "gpt-5.6-sol-wm"}})
			groupID := int64(12)
			repo := &modelsListAccountRepoStub{byGroup: map[int64][]Account{groupID: {account}}}
			svc := &GatewayService{accountRepo: repo}
			require.Equal(t, []string{"auto", "gpt-5.6-sol-wm"}, svc.GetAvailableModels(context.Background(), &groupID, PlatformOpenAI))

			// A real unmapped non-Web account still contributes the standard
			// catalog, even when the pool also contains Web accounts.
			for _, nonWebType := range []string{AccountTypeOAuth, AccountTypeAPIKey} {
				repo.byGroup[groupID] = []Account{account, {ID: 2, Platform: PlatformOpenAI, Type: nonWebType}}
				want := append([]string{"auto", "gpt-5.6-sol-wm"}, openai.DefaultModelIDs()...)
				require.ElementsMatch(t, dedupeAndSortModelIDs(want), svc.GetAvailableModels(context.Background(), &groupID, PlatformOpenAI))
			}
		})
	}
}
